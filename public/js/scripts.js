...
// === IMAGE LOADER UTILITY (Add near top of scripts.js) ===
function initLazyImages(container = document) {
  const images = container.querySelectorAll('img[loading="lazy"]:not(.loaded)');
    ...
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const onError = () => {
          ...
          img.classList.add('loaded'); // Still fade in (show broken icon or fallback)
          observer.unobserve(img);
        };
        ...
        observer.unobserve(img); // No need to observe further
      }
    });
  }, {
